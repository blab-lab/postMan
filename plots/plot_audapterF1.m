function [h] = plot_audapterF1(data, p, bInterpret)
...
for nax = 1:ncols
    ...
    if p.bOutline
        plot(tAxis/fs,data(nax).fmts(:, 1), 'Color','w','LineWidth',p.fmtsLineWidth+.5);
        plot(tAxis/fs,data(nax).sfmts(:, 1), 'Color','w','LineWidth',p.sfmtsLineWidth+.5);
    end
    plot(tAxis/fs,data(nax).fmts(:, 1), 'Color',p.fmtsColor,'LineWidth',p.fmtsLineWidth);
    plot(tAxis/fs,data(nax).sfmts(:, 1), 'Color',p.sfmtsColor,'LineWidth',p.sfmtsLineWidth);
