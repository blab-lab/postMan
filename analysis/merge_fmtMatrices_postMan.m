function [] = merge_fmtMatrices_postMan(dataPath,plotfile,conds2merge,mergednames,bSaveCheck)
...
for c2m = 1:length(conds2merge)
    ...
    %set analysis length depending on if baseline or shift trial
    if ~contains(conds2merge{c2m}, 'No', 'IgnoreCase', true) %shift trial
        anlLength = length(analyses);
    elseif contains(conds2merge{c2m}, 'No', 'IgnoreCase', true) %baseline trial 
        anlLength = 2;
    end
    ...
    fmtData.percNaN.(newcnd) = get_percNaN(fmtMatrix.rawf1.(newcnd));
