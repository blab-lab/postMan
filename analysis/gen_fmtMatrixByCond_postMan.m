function [filename] = gen_fmtMatrixByCond_postMan(dataPath,indBase,indShift,dataValsStr,trialType, bMels,bFilt,bSaveCheck)
...
%get sampling rate
if ~isfield('ftrack_taxis', dataVals)
    tstep = 0.004;
    fs = 1/tstep;
else
...
for c = 1:length(indShift) % for each condition to plot
    ...
    if indShift(c).inds 
        ...
        onsetMeanf1 = nanmean(diff1.(conds{c})(floor(fs*0.025):20,:),1);
        ...
        if isfield(indShift,'shiftind')
           ...
           rawf1 = rmfield(rawf1, (conds{c}));
           rawf2 = rmfield(rawf2, (conds{c}));
           count = count + 1;
       end
    end
...
if ~strcmp(expt.name, 'cat')
    [~,tstep] = get_fs_from_taxis(dataVals(goodtrials(1)).ftrack_taxis); %#ok<ASGLU>
end
%% save data
if length(indShift) < 12
    filename = sprintf('%s_fmtMatrix_%s_%s.mat', trialType, [indShift.name],basename);
else 
    filename = sprintf('%s_fmtMatrix_%s.mat', trialType, 'postMan');
end
